...
public class AuditLogController {
	...
	public String list(@RequestParam(required = false) Long after, @RequestParam(required = false) String afterAt,
		...
		CursorPage<AuditLogSummary> page = before != null ? auditService.findPreviousPage(cursor, PAGE_SIZE)
				: auditService.findNextPage(cursor, PAGE_SIZE);

		List<AuditLogSummary> content = page.content();
		...
		if (!content.isEmpty()) {
			...
			AuditLogSummary first = content.get(0);
			AuditLogSummary last = content.get(content.size() - 1);
